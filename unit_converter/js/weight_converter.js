const len_units = ["mg","g"];

const weight_form = document.getElementById("Weight-form");
...
weight_form.addEventListener("submit",(e)=>{
    ...
    const unit_from = weight_form.unit_from.value;
    const unit_to = weight_form.unit_to.value;
    const weight = weight_form.length_value.value;

    console.log(`Convert from  ${weight} ${unit_from} to ${unit_to}`)
    try{
        if(unit_to === unit_from){
            throw new Error("Units cannot be the same");
        }
        let result = Convert(unit_from,unit_to,weight);
        console.log("Result: " + result);

        converter_page.style.display = "none";
        result_page.style.display = "block";
        from_id.innerText = `${weight} ${unit_from}`;
